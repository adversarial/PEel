...
TEB* EXPORT LIBCALL KpGetCurrentTib() {
  #if x86
	return (TEB*)__readfsdword(0x18);
  #else
	return (TEB*)__readgsdword(0x18);
  #endif
}
...
wchar_t* EXPORT LIBCALL KpGetFilePath(IN const PEB* pPeb) {
	return pPeb->ProcessParameters->ImagePathName.Buffer;
}

DWORD EXPORT LIBCALL KpGetCurrentProcessId(IN const TEB* pTeb) {
	return *(DWORD*)((DWORD_PTR)pTeb + 0x20);
}

DWORD EXPORT LIBCALL KpGetCurrentThreadId(IN const TEB* pTeb) {
	return *(DWORD*)((DWORD_PTR)pTeb + 0x24);
}

DWORD KpGetLastError(IN const TEB* pTeb) {
	return *(DWORD*)((DWORD_PTR)pTeb + 0x34);
}

wchar_t* EXPORT LIBCALL KpGetEnvironmentVariable(PEB* pPeb, const wchar_t* wzVar) {
	MEMORY_BASIC_INFORMATION mbi = {0};
	wchar_t* wzInternal = *(wchar_t**)((DWORD_PTR)pPeb->ProcessParameters + offsetof(PROCESS_PARAMETERS, Environment));
	VirtualQuery(wzInternal, &mbi, sizeof(MEMORY_BASIC_INFORMATION));
	do {
		if (!wcsnicmp(wzInternal, wzVar, wcslen(wzVar))) {
			return (wchar_t*)((DWORD_PTR)wcschr(wzInternal, L'=') + sizeof(wchar_t));
		}
		wzInternal = (wchar_t*)((DWORD_PTR)wzInternal + (wcslen(wzInternal) + 1) * sizeof(wchar_t));	// next var
	} while ((DWORD_PTR)wzInternal < mbi.RegionSize + (DWORD_PTR)mbi.BaseAddress);
	return NULL;
}

HANDLE KpGetStdHandle(IN const PEB* pPeb, const DWORD nStdHandle) {
	HANDLE hStd = NULL;
	switch(nStdHandle) {
		case STD_INPUT_HANDLE:
			hStd = *(HANDLE*)((DWORD_PTR)pPeb->ProcessParameters + offsetof(PROCESS_PARAMETERS, StandardInput));
			break;
		case STD_OUTPUT_HANDLE:
			hStd = *(HANDLE*)((DWORD_PTR)pPeb->ProcessParameters + offsetof(PROCESS_PARAMETERS, StandardOutput));
			break;
		case STD_ERROR_HANDLE:
			hStd = *(HANDLE*)((DWORD_PTR)pPeb->ProcessParameters + offsetof(PROCESS_PARAMETERS, StandardError));
			break;
	}
	return hStd;
}