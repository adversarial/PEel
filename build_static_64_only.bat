...
:: cleanup     
     FOR %%i in (*.o) DO del %%i
     IF EXIST ..\Release\PEel32Plus.lib del ..\Release\PEel32Plus.lib /Q

cd .\peel\
     FOR %%i in (*.c) DO (gcc -c %%i -std=c99 -Os -s -DSUPPORT_PE32PLUS=1 -DSUPPORT_PE32=0 & ECHO Compiling %%i)
     ...
     ar rcs ..\Release\PEel32Plus.lib %objects%
